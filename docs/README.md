...
- [`RELEASE_EVIDENCE_TEMPLATE.md`](RELEASE_EVIDENCE_TEMPLATE.md) — structured evidence record for exact commit/workflow/artifact plus native device/system observations; Web-specific browser/hosting evidence should additionally reference `WEB_QA_CHECKLIST.md` until the template is expanded further.
...
- [`RELEASING.md`](RELEASING.md) — stable-release procedure, source preparation, branding, automated/manual gates, manual-QA JSON structural review, signing boundaries, candidate builds, visual review, tagging, and publication rules. The current Web-specific release gates are additionally defined in `WEB_SUPPORT.md`, `WEB_QA_CHECKLIST.md`, and `TODO.md`.
...
- [`UNSIGNED_ARTIFACTS.md`](UNSIGNED_ARTIFACTS.md) — purpose and limitations of non-production/unsigned/no-codesign release-candidate artifacts; current Web static-bundle candidate behavior is governed by `RELEASE_CANDIDATE_MANIFEST.md` and `WEB_SUPPORT.md`.
