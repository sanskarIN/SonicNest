...
- Key the stateful microphone `DropdownButtonFormField` by the selected browser device ID. Flutter's `FormField.initialValue` initializes/reset the field rather than continuously tracking later state, so remounting the form field when `_selectedDevice` changes keeps the displayed browser microphone synchronized after device refresh/removal or explicit selection.
...
- selection-keyed microphone form-field state synchronized with `_selectedDevice`;
...
These repository checks protect source-level behavior. They do not replace real browser testing. Manual QA still needs representative Chromium, Firefox, and Safari/WebKit coverage for device enumeration/removal, selected-device display after refresh, stream termination, input changes, playback pause/resume, source switching, injected/real playback failures, and recovery after browser/media-device interruptions.
