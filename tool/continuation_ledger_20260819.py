#!/usr/bin/env python3
"""Apply the 2026-08-19 SonicNest QA-evidence-transfer ledger updates.

This one-shot maintainer helper is intentionally strict and idempotent. It fails
when a required anchor has drifted so the validation workflow cannot silently
write documentation in the wrong location.
"""
...
def update_project_state() -> None:
    ...
    insert_after(
        ...
        "  - same-version Manual QA JSON sharing and strict non-destructive import/merge with complete-catalog privacy timeline summary validation explicit confirmation and a 2 MiB input bound\n"
    ...
    insert_after(
        ...
        "  latest_timeline_hardening_commit: 1797555cc5771916d6e34ae49689f6bd76441dbe\n",
    ...
    replace_once(
        ...
        "    - strict schema app identity privacy catalog metadata timeline and summary validation\n"
...
def update_changelog() -> None:
    insert_after(
        "CHANGELOG.md",
        "### Added\n",
        "- Same-version Manual QA evidence JSON sharing and strict import/merge for consolidating assessed release checks across test targets, with complete-catalog/privacy/timeline/summary validation, an explicit merge preview, and a 2 MiB input limit.\n"
        "- Regression coverage for evidence timeline bounds, monotonic local status timestamps, newest-result merge semantics, incompatible import rejection, transfer localization, and screen/picker integration.\n",
    ...
    insert_after(
        ...
        "- The current import/export dependency ledger is aligned with `file_picker 12.0.0-beta.7`, `share_plus 13.3.0`, and `wakelock_plus 1.7.0`.\n",
    ...
    insert_after(
        ...
        "- Imported `notRun`, older, or equal-timestamp QA entries no longer clear or overwrite newer locally assessed evidence.\n",
        "Malformed persisted QA sessions can no longer retain backward",
    )
...
def update_release_notes() -> None:
    insert_after(
        ...
        "- Import validates the current bundle/session schema, SonicNest identity/version, fixed privacy contract, complete QA catalog and metadata, chronological timestamps, and recomputed summary before showing add/update/ignored counts for confirmation. Imported `notRun`, older, and equal-timestamp results never destructively clear newer local assessed evidence.\n"
        "- Local QA evidence timestamps are monotonic even across backward device-clock changes, and malformed persisted result timestamps outside the session timeline are discarded before they can be re-exported.\n",
...
def update_roadmap() -> None:
    insert_after(
        ...
        "- Same-version Manual QA JSON transfer is implemented with explicit JSON sharing, strict import validation, newest-assessed-result-only merge semantics, a 2 MiB file bound, and user confirmation before persistence.\n"
    ...
    insert_after(
        ...
        " The ledger now also supports explicit JSON file sharing and exact-current-version JSON import with strict catalog/privacy/timeline/summary validation, a bounded 2 MiB input, merge-count confirmation, and non-destructive newest-assessed-result-only consolidation.",
...
def update_todo() -> None:
    insert_after(
        ...
        "- [x] Add explicit Manual QA JSON sharing plus exact-current-version strict import/merge with complete-catalog/privacy/timeline/summary validation, a 2 MiB input bound, monotonic timestamps, merge preview, and newest-assessed-result-only persistence.\n",
...
- Added exact SonicNest app-version matching, fixed privacy-contract validation, chronological bundle/session/result validation, and recomputed summary consistency checks before any merge is offered.
...
- Added focused model, import-service, validation, localization, and source-integration regressions for the transfer path.
- Updated Manual QA evidence documentation with the transfer workflow and explicit manual-gate boundary.
- Synchronized the project dependency ledger with `file_picker 12.0.0-beta.7`, `share_plus 13.3.0`, and `wakelock_plus 1.7.0` from `pubspec.yaml`.

### Validation boundary

This continuation ledger is committed only after the scoped maintenance runner completes canonical Dart formatting, `flutter analyze --no-fatal-infos`, and the complete `flutter test` suite successfully on the same local commit chain. The runner does not mark physical-device, accessibility, stress, signing, notarization, store-console, or stable-release gates complete.

### Key implementation commits

- `1d6809112ff39200e2749379bd92be84b3bcd983` — persisted QA timeline invariants and newest-result merge primitive.
- `381b6ebf2ce9718491c9575f8ad3e7799d924ef3` / `e64600b143305b607bdd907cf3f91720e31078cc` — strict evidence import service and status-count correction.
...
- `4370a817c2b59d83020c5081731d77cf5baea659`, `ed4f0c51bc85e486cbfc1e810fd1ceecb5477de8`, `9b6933e1c43ec6d69bcce0452adfee121ac9a3a0`, `cf28fdced27b0c0e6d38b5d7f38af5b557ca6c82`, `29688d02d925976b99b721e8111586d727607881`, `dbc8fb6461dc0159004aee595ad6decb58384398`, and `1e5ff095fba929dd99cac935f9de14f48fed1416` — focused regression coverage.

### Release status

SonicNest remains a development preview. The remaining unchecked `TODO.md` items still require the real devices/systems, sustained workloads, representative media, accessibility tooling, maintainer-owned signing credentials, distribution-console access, translation review, or final release approval described there.
""",
        "## 2026-08-19 — Manual QA evidence transfer and timeline hardening",
    )
