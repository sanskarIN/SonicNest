...
class FinalRepositoryContractTest(unittest.TestCase):
    ...
    def test_resolved_v12_plugin_and_export_contract_is_tracked(self) -> None:
        ...
        self.assertNotIn("allowMultiple:", external)
        self.assertNotIn("FilePicker.saveFile(", external)
        self.assertIn("FilePicker.pickFiles(", external)
        self.assertIn("FilePicker.pickFile(", external)
        self.assertIn("FilePicker.getDirectoryPath(", external)
        self.assertIn("_collisionSafeDestination", external)
